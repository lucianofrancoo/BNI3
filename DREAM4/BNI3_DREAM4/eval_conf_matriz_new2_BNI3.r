...
evaluar_red <- function(ruta_reglas, size, size_num, method_name, ruta_gold_standard, ruta_matriz_bin) {
  ...
  tryCatch({
    ...
    # Tomar la primera regla de cada gen (ya vienen ordenadas por Score descendente)
...
for(size_num in 1:5) {
  ...
  for(method_dir in method_dirs) {
    ...
    rules_file <- file.path(method_dir, "rules_by_gene_evaluated.tsv")
    
    if(!file.exists(rules_file)) {
      cat(sprintf("  Saltando %s (no tiene rules_by_gene_evaluated.tsv)\n", method_name))
      next
    }
...
cat("\n========== RESUMEN POR MÉTODO DE BINARIZACIÓN Y MUESTREO ==========\n")
...
resumen <- resultados_df %>%
  group_by(Binarization, Tipo_Muestreo) %>%
  summarise(
    N_casos = n(),
    ...
    DynAcc_medio = mean(DynamicAccuracy, na.rm = TRUE),
    .groups = 'drop'
  )

cat("\nComparación por método de binarización y timepoints:\n")
print(resumen)

# Resumen adicional solo por tipo de muestreo (agregando ambos métodos)
cat("\n========== RESUMEN GENERAL POR TIPO DE MUESTREO ==========\n")

resumen_global <- resultados_df %>%
...
cat("\nComparación 21 vs 5 timepoints (todos los métodos):\n")
